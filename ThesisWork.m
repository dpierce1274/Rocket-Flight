...
Diameter = 98;         % Body Tube Diameter of Rocket (mm)
d = Diameter/1000;     % Body Tube Diameter of Rocket (m)
...
rhos = .01841;       % Density of air in kg/m^3 at 20 deg. Centigrade
A = pi*(d*.5)^2;    % Cross-sectional Area of rocket (m^2)
g0 = 9.807;         % Gravity constant (m/s^2)

%% Boost Phase

tb = linspace(0,60,1000);      % Boost phase time
...
[z,y] = max(h);
hfeet = z*3.28084;
disp(['Maximum height is ',num2str(z),'(m) or ',num2str(hfeet),' at ',num2str(tb(y)),'(s)'])
[z,y] = max(v);
